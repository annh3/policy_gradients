...
class TestBasic(unittest.TestCase):
    ...
    def test_baseline_op(self):
        ...
        for i in range(1000):
            #print("i: ", i)
            #weights_1 = self.baseline.save_weights()
            self.baseline.update_baseline(returns, self.rand_obs)
            #weights_2 = self.baseline.save_weights()
            #deltas = self.baseline.calculate_deltas(weights_1, weights_2)
            #print("PRINTING DELTAS: ",  deltas)
